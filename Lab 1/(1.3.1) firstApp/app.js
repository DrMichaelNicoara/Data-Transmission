...
document.getElementById("message").innerHTML = "Message by Nicoara Mihai from JavaScript";
...
$.fn.hasOwnNestedProperty = function (propertyPath) {
    if (!propertyPath)
        return false;

    var properties = propertyPath.split('.');
    var obj = this.get(0); // extract the underlying object from the jQuery collection
...
function printUserAttribute(attrib) {
    property = $(user).hasOwnNestedProperty(attrib)
    if (property) {
        return property;
    }
    else {
...
console.log("Domain of user: " + printUserAttribute("company.domain"))